...
test('Margelo and Expensify details show the dates and public work', async ({ page }) => {
  ...
  await expect(page.locator('.detail-prose')).toContainText('since 2021')
  await expect(page.locator('.detail-prose')).toContainText('C++, Swift, and Kotlin')
...
test('all six authored objects share the main collection and Off the clock links reading and music', async ({ page }) => {
  ...
  await expect(page.locator('.detail-prose')).toContainText('classical piano')
  await expect(page.locator('.detail-prose')).toContainText('coding challenges')
...
test('native detail explains the library work and NitroFetch integration', async ({ page }) => {
  ...
  await expect(page.locator('.detail-prose')).toContainText('For NitroFetch, I added prefetching to Expensify')
  await expect(page.locator('.detail-prose')).toContainText('I wrote about what changed')

...
test('larger text grows the header and keeps detail content below it', async ({ page }) => {
  ...
  await expect.poll(() => page.locator('.view-layer').evaluate(element => element.getBoundingClientRect().top)).toBeLessThan(detail!.y - 40)
...
test('mobile header rises with collection and detail scrolling', async ({ page }) => {
  ...
  await expect.poll(() => header.evaluate(element => element.getBoundingClientRect().top)).toBeLessThan(-40)
  const scrolledHeader = await header.boundingBox()
  const scrolledNavigation = await navigation.boundingBox()
  expect(scrolledNavigation!.y).toBeLessThan(initialNavigation!.y - 40)
  expect(scrolledNavigation!.y).toBeGreaterThanOrEqual(30)
  expect(scrolledHeader!.y + scrolledHeader!.height).toBeLessThan(initialHeader!.height - 40)
  ...
  await expect.poll(() => layer.evaluate(element => element.getBoundingClientRect().top)).toBeLessThan(initialLayerTop - 40)

...
import { collectionPose, splitFeatured, validateCollection } from './layout'
...
describe('collection layout', () => {
  ...
  it('generates repeatable poses inside the reserved movement range', () => {
    for (const seed of [0, 1, 2, 100]) {
      ...
      expect(Math.abs(pose.x)).toBeLessThanOrEqual(8)
      expect(Math.abs(pose.y)).toBeLessThanOrEqual(8)
